====================================
Tutorial: connect to a Wi-Fi hotspot
====================================
...
Right after installing Qubes OS, connecting to a Wi-Fi hotspot might be the first thing you might want to do. Even if you don't want to browse internet, this would allow you to update the system.
...
In order to start, **let's power-on the device where Qubes OS is installed**. A succession of screens are presented to you, we will take an overview of them:

1. **the firmware** of your computer (that might use the :program:`BIOS` or the :program:`UEFI`), a piece of software independent from Qubes OS. How the screen renders is dependent of the hardware.

2. then a screen that provides a list of options, the first one being :guilabel:`Qubes, with Xen hypervisor`. If you do nothing, this option is automatically selected. This piece of software is a **bootloader**. That specific one is called :program:`GRUB`:

   .. image:: /attachment/tutorials/connect-to-wifi/grub.png
      :alt:
      :width: 100%

3. A prompt appears and you must provide the passphrase used to **encrypt your drive**. Without that, it is impossible for the system to decrypt the drive, and thus, read your data. When you access to this screen, Qubes OS is already running.
...
You don't have to remember the name of the different softwares or even the order of the steps. Just be aware of the process. Qubes OS is **a collection of tools**, build upon existing softwares, like GRUB. GRUB is the tool that will load another software called Xen. Xen does not provide any graphical output as "Xen", so we will not learn more about it now.

Connect to a Wi-Fi hotspot
--------------------------

After login in, the first screen you get displays a full width panel at the top of the screen. On the right, you can see a group of icons, one of them **being red** and displaying two superposed screens. **Click that icon.**

.. image:: /attachment/tutorials/connect-to-wifi/click-on-nm-applet.png
   :alt:
   :width: 100%

A menu will drop down: notice that it has **red borders**. You should be able to select your hotspot in the list of the available hotspots:
...
.. note::
   ...
   If you don't have a hotspot to connect to, or no Wi-Fi controller, you can use :guilabel:`Create New Wi-Fi Network` or :guilabel:`VPN Connections` to see the next window, then select :guilabel:`Cancel`.
...
To isolate different kind of software, we need to use different qubes (compartments). Most of the time you will have several qubes running at the same time, you will start some other qubes, stop another, etc. One convenient but limited tool to do that is the :program:`Qubes Domains widget`. Here, the term "domain" has quite the same meaning as the term "qube", which we prefer to use. This tool is accessible from **the panel, at the top of the screen**. It resides precisely in the system tray, **at right of the panel**, with its own icon:
...
**To see the currently running qubes**, click the Qubes Domains widget icon, next to the red icon we used to connect to the hotspot.
...
* **to display all the windows of every qube** as if they weren't isolated: *dom0* is also a :term:`GUI domain`.
...
.. image:: /attachment/tutorials/connect-to-wifi/qubes-manager.png
   :alt:
   :width: 100%

The program :program:`Qubes Manager` lists all the existing qubes, not only the running ones. There is much information here, we will ignore most of it for now.
...
Now, you need to go to the :guilabel:`Advanced Options` section: click :guilabel:`Show advanced settings` and select :guilabel:`Provides network to other qubes`. This is the property that allows a qube to serves as a net qube for other qubes.
...
Now, it is time to reopen :program:`Qube Manager` and check that your qube is listed here. You can now start your qube, it is like starting another computer.
...
* click the line corresponding to *sys-net-tutorial* and then:
  ...
  * click the :guilabel:`Start/Resume` button:
    ...
    .. image:: /attachment/tutorials/connect-to-wifi/qubes-manager-start-button.png
       :alt:
       :width: 100%
...
* you can also right click the line corresponding to *sys-net-tutorial* and select :guilabel:`Start/Resu&me`
    ...
    .. image:: /attachment/tutorials/connect-to-wifi/qubes-manager-start-right-click.png
       :alt:
       :width: 100%
...
If :program:`Qube Manager` is not opened, you can quickly check that using :program:`Qubes Domains widget`:

.. image:: /attachment/tutorials/connect-to-wifi/domains-widget.png
   :alt:
   :width: 100%

Why *sys-net-tutorial* is not working like *sys-net*?
-----------------------------------------------------

You might have noted that a new green icon, like the red one used by *sys-net* is now present in the tray. We have a nice imitation of the *sys-net* qube but, **we are about to fail our goal!** Click the green icon and try to connect to your Wi-Fi hotspot, just as we did with *sys-net* in the first place: **you cannot!** The widget display a message telling you :guilabel:`No network devices available`. All you can do is to click :menuselection:`VPN Connections --> Add a VPN connection...`. You get a useless window:
...
* click the line corresponding to *sys-net* and then:
  ...
  * click the :guilabel:`Settings` button:
    ...
    .. image:: /attachment/tutorials/connect-to-wifi/qubes-manager-settings-button.png
       :alt:
       :width: 100%
...
* you can also right click the line corresponding to *sys-net* and select :guilabel:`S&ettings`
    ...
    .. image:: /attachment/tutorials/connect-to-wifi/qubes-manager-settings-right-click.png
       :alt:
       :width: 100%
...
A new window opens: the title is :guilabel:`[Dom0] Settings: sys-net` because its up to the :term:`admin qube` to manage the settings of a qube. If a qube could manage its own settings that would mean that a compromised qube could change its name, label, network access, and so on. Go to the :guilabel:`Devices` tab:

.. image:: /attachment/tutorials/connect-to-wifi/sys-net-devices-tab.png
   :alt:
   :width: 100%

You will see two lists. The first one, on the left, present all the :guilabel:`Available devices`. Those devices correspond to the hardware of your computer so what is listed on your screen might be different from the following screenshot:
